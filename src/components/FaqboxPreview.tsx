...
export default function({data}:Props){
  ...
  return(
    <>
    {theme===1 && (
      <div className="flex flex-col w-[400px] min-h-[850px] absolute  origin-top-left sm:scale-[0.625] 2xl:scale-[0.875] align-middle sm:max-w-md justify-between py-16 px-10 mx-auto bg-white">
    ...
    {theme === 2 && (
      <div className="flex flex-col w-[400px] min-h-[850px] absolute  origin-top-left scale-[0.625] align-middle sm:max-w-md justify-between py-16 px-10 mx-auto bg-blue-300">
        <div className="mx-auto flex flex-col items-center">
          <div className=" w-36 h-36 shadow-md rounded-full relative z-0">
            <Image
              src={logo}
          ...
          <button className="bg-blue-700 text-slate-50 rounded-full w-full py-2 px-5 mt-3 font-semibold hover:bg-blue-800">More question?</button>
          <section>
            {
              dataSearched.map((faq) => (<AccordionItem key={faq.id} question={faq.q} answer={parse(faq.a)}/>))
            }
